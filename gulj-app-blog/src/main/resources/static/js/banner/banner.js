$(function(){
	...
	function slide(slideMode){//轮播方法
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				next.stop(true,true).animate({width:"450px",height:"180px",top:"60px","left":"630px","opacity":0},timer,function(){next.find("span").css("opacity",0); isMoving = false;});
				//prev  -->  curr     prev中的图片li轮换到curr的位置      其他一次轮换
				prev.find("span").css("opacity",0);
				$(".main_banner_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				prev.stop(true,true).animate({width:"550px",height:"280px",left:"155px",top:0,opacity:1},timer,function(){
				...
				$(".main_banner_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				next.stop(true,true).animate({width:"550px",height:"280px",left:"155px",top:0,opacity:1},timer,function(){
					$(this).find("p").animate({"bottom":"0px"});	//当前这张图片的标题运动出来
				});
	...
	if(len>3){
		...
		$(".btn_list span").click(function(event){
			...
			slide(flag);
			intervaltimer=setInterval(slide,10000);//自动轮播
		...
		$(".main_banner_box li").on("mousemove",function(){
			if($(this).css("width")=="670px"){//鼠标移入为当前正中显示的图片li，则清除定时器
				...
				clearInterval(intervaltimer);
				intervaltimer=null;
				intervaltimer=setInterval(slide,10000);
		});
